...
async function callApiWithRetries(url, options) {
  ...
  while (attempt < NxConstants.maxRetries) {
    updateStatus(
      attempt === 0
        ? NxConstants.strings.generating
        : NxConstants.strings.retrying
    );
    ...
    const delayIndex = Math.min(
      attempt - 1,
      NxConstants.retryDelays.length - 1
    );
    const baseDelay = NxConstants.retryDelays[delayIndex];
    const jitter =
      Math.floor(
        Math.random() * (NxConstants.jitterMax - NxConstants.jitterMin + 1)
      ) + NxConstants.jitterMin;
...
function buildFilename(candidate, ext) {
  ...
  const indexOrSeed =
    typeof candidate.seed !== "undefined" ? candidate.seed : candidate.index;
  const parts = [];
...
async function saveAllOrSelected(selectedOnly) {
  ...
  const zipName = `nexusai_results_${new Date()
    .toISOString()
    .replace(/[:\.]/g, "")}.zip`;
...
async function generateHandler() {
  ...
  let url =
    NxConstants.baseUrl +
    NxConstants.routeTemplate.replace("{model}", encodeURIComponent(model));
...
function init() {
  ...
  document
    .getElementById("set-default-model")
    .addEventListener("click", setDefaultModel);
  document.getElementById("mode").addEventListener("change", () => {
    toggleModeOptions();
    setDefaultModel();
  });
  document
    .getElementById("size-preset")
    .addEventListener("change", toggleCustomSize);
  document
    .getElementById("generate")
    .addEventListener("click", generateHandler);
  document.getElementById("cancel").addEventListener("click", cancelHandler);
  document
    .getElementById("save-selected")
    .addEventListener("click", () => saveAllOrSelected(true));
  document
    .getElementById("save-all")
    .addEventListener("click", () => saveAllOrSelected(false));
  document
    .getElementById("clear-previews")
    .addEventListener("click", clearPreviews);

  // Add ListModels test button for debugging
  const listModelsBtn = document.createElement("button");
  listModelsBtn.textContent = "List Models (Debug)";
  listModelsBtn.style.marginTop = "0.5rem";
  listModelsBtn.addEventListener("click", listModelsHelper);
  document.getElementById("actions-section").appendChild(listModelsBtn);
}

// Helper to list available models for the API key
async function listModelsHelper() {
  const apiKeyInput = document.getElementById("api-key");
  const key = apiKeyInput.value.trim();
  if (!key) {
    showError("API key required for ListModels", "");
    return;
  }
  ...
  try {
    ...
    alert("ListModels response logged to console.");
  } catch (e) {
    console.error("ListModels failed", e);
    showError("ListModels failed", e.message || "");
  }
}
